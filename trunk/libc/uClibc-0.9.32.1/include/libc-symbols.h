 ...
 */
/* BRCM, to resolve __uClibc_main.o can't refernece __preinit_array_* 
 * problem, we need to define visibility of attribute_hidden to default.
  */ 
#if (defined __GNUC__ && defined __GNUC_MINOR__ \
	&& (( __GNUC__ >= 3 && __GNUC_MINOR__ >= 3) || __GNUC__ >= 4) \
    ) || defined __ICC
# define attribute_hidden __attribute__ ((visibility ("default")))
# define attribute_protected __attribute__ ((visibility ("default")))
# define __hidden_proto_hiddenattr(attrs...) __attribute__ ((visibility ("default"), ##attrs))
#else
